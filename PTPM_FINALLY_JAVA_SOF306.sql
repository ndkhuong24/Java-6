USE [master] CREATE DATABASE [PTPM_FINALLY_JAVA_SOF306]
GO
...
GO
	CREATE TABLE TheThanhVien(
		...
		NgayHuy DATE,
		TrangThai INT,
		NguoiSoHuu BIGINT REFERENCES KhachHang(MaKhachHang)
	)
GO
...
GO
	CREATE TABLE DiaChi (
		MaDiaChi UNIQUEIDENTIFIER DEFAULT NEWID() PRIMARY KEY,
...
GO
	CREATE TABLE HoaDon(
		...
		NgayThanhToan DATE,
		TrangThai INT,
		NguoiMua BIGINT REFERENCES KhachHang(MaKhachHang),
	)
GO
	CREATE TABLE HoaDonChiTiet(
		MaHoaDon BIGINT REFERENCES HoaDon(MaHoaDon),
...
GO
	CREATE TABLE PhieuGiaoHang(
		...
		PhiGiaoHang MONEY,
		HoaDonGiao BIGINT REFERENCES HoaDon(MaHoaDon),
		DiaChiGiao UNIQUEIDENTIFIER REFERENCES DiaChi(MaDiaChi),
		GhiChu NVARCHAR(MAX),
		TrangThai INT,
	) CREATE TABLE GioHang(
		MaGioHang UNIQUEIDENTIFIER DEFAULT NEWID() PRIMARY KEY,
		NgayTao DATE,
		NgayCapNhap DATE,
		NguoiSoHuu BIGINT REFERENCES KhachHang(MaKhachHang),
		GhiChu NVARCHAR(MAX),
...
GO
	CREATE TABLE GioHangChiTiet(
		MaGioHang UNIQUEIDENTIFIER REFERENCES GioHang(MaGioHang),
		MaSanPham BIGINT REFERENCES SanPham(MaSanPham),
		SoLuong INT,
		GhiChu NVARCHAR(MAX),
		TrangThai INT,
...
GO
	CREATE TABLE YeuThichChiTiet(
		MaDanhSach UNIQUEIDENTIFIER REFERENCES DanhSachYeuThich(MaDanhSach),
		MaSanPham BIGINT REFERENCES SanPham(MaSanPham),
		GhiChu NVARCHAR(MAX),
		TrangThai INT,
